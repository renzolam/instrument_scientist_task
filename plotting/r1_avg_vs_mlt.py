...
def get_avg_vs_mlt(
    ...
    low_lat_lim, high_lat_lim = plot_params.r1_avg_vort_vs_mlt_aacgm_lat_lim

    season_vorts = np.array([vort_data
    ...
    lat_coords = np.array([vort_data.aacgm_lat_c for vort_data in season_vorts], dtype=float)
    vort_vals = np.array([vort_data.vorticity_mHz for vort_data in season_vorts], dtype=float)
    ...
    lat_edges = plot_utils.create_bin_edges((low_lat_lim, high_lat_lim), d_lat)
    ...
    vort_sum = np.sum(two_d_stats['sum'], axis=1)  # Sum of all vorticity measurement for a given MLT
    count_sum = np.sum(two_d_stats['count'], axis=1)  # Total number of data points for a given MLT
...
def formatting(
    ...
    lat_min, lat_max = plot_params.r1_avg_vort_vs_mlt_aacgm_lat_lim
    ...
    ax.set_yticks(
        np.arange(-3.5, 3.5, 0.5),
        minor=False
    )

    ax.set_yticks(
        np.arange(-3.5, 3.5, 0.1),
        minor=True
    )
